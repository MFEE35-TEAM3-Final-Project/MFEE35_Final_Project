...
const { userPassport } = require("../models/passport.js");
...
app.post('/cart/add', userPassport, async (req, res) => {
  try {
    ...
    // 檢查是否已經存在相同的記錄
    const checkQuery = `SELECT * FROM shopping_cart WHERE user_id = ? AND product_id = ?`;
    const checkQuery = `SELECT * FROM shopping_cart WHERE user_id = ? AND product_id = ?`;
    query(checkQuery, [user_id, product_id], (error, results) => {
      if (error) {
        console.error(error);
        res.status(500).json({
          success: false,
      ...
      if (results.length > 0) {
        ...
        query(updateQuery, [quantity, user_id, product_id], (error, results) => {
          if (error) {
            console.error(error);
            res.status(500).json({
              success: false,
      ...
      query(insertQuery, [user_id, product_id, quantity], (error, results) => {
        if (error) {
          console.error(error);
          res.status(500).json({
            success: false,
